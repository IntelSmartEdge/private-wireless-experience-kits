...
cp -rf "$helm_charts_home" /tmp/"$backup"
upf_des_path=$helm_charts_home/cn/charts/upf/config/
cu_des_path=$helm_charts_home/cu/config/
phy_des_path=$helm_charts_home/du/l1config/
du_des_path=$helm_charts_home/du/l2config/

#copy wrapper.sh to config dir
cp -Lrf "$helm_charts_home"/upf.*  "$upf_des_path"/wrapper.sh
cp -Lrf "$helm_charts_home"/cu.*   "$cu_des_path"/wrapper.sh
cp -Lrf "$helm_charts_home"/phy.*  "$phy_des_path"/wrapper.sh
cp -Lrf "$helm_charts_home"/du.*   "$du_des_path"/wrapper.sh

#upf setting
sed -i -e "/limits:/a\                cpu: $upf_core_num" "$helm_charts_home"/cn/charts/upf/templates/deployment.yaml
sed -i -e "/requests:/a\                cpu: $upf_core_num" "$helm_charts_home"/cn/charts/upf/templates/deployment.yaml
